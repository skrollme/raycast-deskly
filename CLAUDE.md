...
`Location` has `id`, `name`, `floors: Floor[]`, and `timeframes: Timeframe[]`. `Timeframe` has `name`, `from`, and `until` (HH:MM strings). `Floor` has `id`, `name`, and `rooms: Room[]`. `Room` has `id` and `name`. This three-level hierarchy is returned by `fetchSpaces()` and drives the cascading dropdowns in `book-a-seat`. Note: the grow API (`fetchSpaces`) and the main API (`fetchInformation`) use different location IDs, so `book-a-seat` merges timeframes from `information.availableLocations` into the spaces tree by location name before rendering the Timeframe dropdown.
